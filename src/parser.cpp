//////////////////////////////////////////////////////////////////////////
//									//
// siv parser methods to parse the tokens into an AST			//
//									//
//////////////////////////////////////////////////////////////////////////
...
TokenBase *TokenREQUIRE::parse(Program &pgm)
{
    TokenBase *tn;

    DBG(std::cout << std::endl << "TokenREQUIRE::parse()" << std::endl);

    tn = pgm.nextToken();
    DBG(cout << "TokenREQUIRE::parse() calling capabilities=parseExpression(" << (char)tn->get() << ')' << endl);
    if ( !(capabilities=pgm.parseExpression(tn, true)) )
	throw "Failed to parse require expression";
...
TokenBase *TokenIF::parse(Program &pgm)
{
    ...
    tn = pgm.nextToken();
    if ( tn->id() != TokenID::tkALLOF && tn->id() != TokenID::tkANYOF )
    {
	DBG(cerr << "TokenIF::parse() expecting either allof or anyof" << endl);
	throw tn;
    }
    tn = pgm.nextToken();
...
TokenBase *TokenELSIF::parse(Program &pgm)
{
    TokenBase *tn;

    DBG(std::cout << std::endl << "TokenELSIF::parse()" << std::endl);

    tn = pgm.nextToken();
    if ( tn->id() != TokenID::tkOpBrk )
    {
	DBG(cerr << "TokenELSIF::parse() expecting (" << endl);
	throw tn;
    }
    DBG(cout << "TokenELSIF::parse() calling condition=parseExpression(" << (char)tn->get() << ')' << endl);
    if ( !(condition=pgm.parseExpression(tn, true)) )
	throw "Failed to parse if expression";

    tn = pgm.nextToken();
    DBG(cout << "TokenELSIF::parse() calling statement=parseStatement(" << (char)tn->get() << ')' << endl);
    if ( !(statement=pgm.parseStatement(tn)) )
	throw "Failed to parse if statement";
	...
	tn = pgm.nextToken(); // get the else
	tn = pgm.nextToken(); // skip the else
	DBG(cout << "TokenELSIF::parse() calling elsestmt=parseStatement(" << (char)tn->get() << ')' << endl);
	elsestmt = pgm.parseStatement(tn);
	if ( !elsestmt )
	    throw "parse error on else";
    }
    ...
    if ( tn )
	DBG(cout << "TokenELSIF::peekToken() type: " << (int)tn->type() << " id: " << (int)tn->id() << ')' << endl);
